...
class SimulationController:
    ...
    def __init__(self, max_round_num, number_of_sheep, xy_limit,
                 sheep_step_length, wolf_step_length, json, csv):
        self.max_round_num = max_round_num
        self.round_count = 1
        self.sheeps = [Sheep(xy_limit, sheep_step_length, x + 1)
    ...
    def make_json_log(self):
        return {'round_no': self.round_count,
                ...
                'sheep_pos': [(sheep.get_x_cord(),
                               sheep.get_y_cord())
                              if sheep.is_alive() else None
                              for sheep in self.sheeps]
                }

    def wolf_position_info(self):
        print("Wolf's position:  ("
              + str(round(self.wolf.get_x_cord(), 3))
              + ", "
              + str(round(self.wolf.get_y_cord(), 3))
              + ")"
              )
    ...
    def start_simulation(self, with_pause=False):
        ...
        for i in range(self.max_round_num):
            ...
            if self.wolf.target.is_alive():
                print("Wolf is chasing the sheep "
                      + str(self.wolf.target.get_sequence_num())
                      )
            else:
                print("Wolf ate the sheep "
                      + str(self.wolf.target.get_sequence_num())
                      )

            # print("Sheep " + str(self.wolf.target.sequence_num)
            # + " position:  ("
            # + str(
            ...
            self.writer.write_to_csv(self.get_round_count(),
                                     self.count_alive_sheeps())

            logging.info("The number of alive sheep: "
                         + str(self.count_alive_sheeps())
                         )
            ...
            if self.count_alive_sheeps() == 0:
                ...
                logging.info("The simulation has terminated because "
                             "there is no alive sheep")

                exit(0)

        logging.info("The simulation has terminated because "
                     "of reaching the maximum number of rounds")
