...
class Wolf(Animal):
    ...
    def sheep_to_wolf_distance(self, sh):
        return ((self.get_x_cord() - sh.get_x_cord()) ** 2
                + (self.get_y_cord() - sh.get_y_cord()) ** 2) ** 0.5
    ...
    def find_closest_sheep(self, sheep):
        ...
        for sh in sheep:
            if not closest.is_alive():
                closest = sh
            if sh.is_alive() and (self.sheep_to_wolf_distance(sh)
                                  < self.sheep_to_wolf_distance(closest)):
                closest = sh
        self.target = closest

    def calculate_angle(self, sheep):
        return math.atan2(self.get_y_cord()
                          - sheep.get_y_cord(), self.get_x_cord()
                          - sheep.get_x_cord())
    ...
    def move(self, sheep=None):
        ...
        if distance <= self.step_length:
            ...
            logging.info("A "
                         + str(self.target.sequence_num)
                         + " sheep was eaten")
        else:
            logging.info("The wolf is chasing sheep "
                         + str(self.target.sequence_num))
        ...
        logging.debug("The wolf moved to position: ("
                      + str(self.get_x_cord()) + ", "
                      + str(self.get_y_cord()) + ")")
